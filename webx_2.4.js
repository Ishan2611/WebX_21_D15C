// Manager Class implementing Employee
var Manager = /** @class */ (function () {
    function Manager(name, id, role, department) {
        this.name = name;
        this.id = id;
        this.role = role;
        this.department = department;
    }
    Manager.prototype.getDetails = function () {
        return "Name: ".concat(this.name, ", ID: ").concat(this.id, ", Role: ").concat(this.role, ", Department: ").concat(this.department);
    };
    return Manager;
}());
// Developer Class implementing Employee
var Developer = /** @class */ (function () {
    function Developer(name, id, role, programmingLanguages) {
        this.name = name;
        this.id = id;
        this.role = role;
        this.programmingLanguages = programmingLanguages;
    }
    Developer.prototype.getDetails = function () {
        return "Name: ".concat(this.name, ", ID: ").concat(this.id, ", Role: ").concat(this.role, ", Languages: ").concat(this.programmingLanguages.join(", "));
    };
    return Developer;
}());
// Creating instances
var manager = new Manager("Ishan", 101, "Manager", "HR");
var developer = new Developer("Joshi", 102, "Developer", ["JavaScript", "TypeScript", "Python"]);
// Display details
console.log(manager.getDetails());
console.log(developer.getDetails());
